...
export default {
  ...
  created() {
    const _this = this
    axios.get('http://58.87.91.31:8088/truck/selectById/' + this.$route.query.id).then(function(resp) {
      console.log(resp)
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://58.87.91.31:8088/truck/insert', _this.ruleForm).then(function(resp) {
            console.log(resp)
